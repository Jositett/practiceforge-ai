...
export function Hammer({ className, size = 24 }: { className?: string; size?: number }) {
  return (
    <svg
      className={className}
...
export function Sparkles({ className, size = 24 }: { className?: string; size?: number }) {
  return (
    <svg
      className={className}
      xmlns="http://www.w3.org/2000/svg"
    ...
    <Tabs defaultValue="rendered" className="h-full flex flex-col">
      <div className="sketchy-card h-full bg-white flex flex-col overflow-hidden p-0">
          ...
          <div className="flex gap-2">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="gap-2 border-2 border-foreground hover:bg-accent">
        ...
        <div className="flex-1 overflow-auto p-8 bg-background/50">
          <TabsContent value="rendered" className="prose prose-slate prose-lg max-w-none focus-visible:outline-none dark:prose-invert m-0">
            <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeHighlight]}>
              {content}
            </ReactMarkdown>
            {isGenerating && (
              <div className="mt-8 flex items-center gap-3 animate-pulse text-primary font-display text-xl">
                <Sparkles size={20} className="animate-spin" />
                Striking the iron...
              </div>
            )}
          </TabsContent>
          <TabsContent value="raw" className="h-full m-0 focus-visible:outline-none">
            <textarea
              readOnly
              value={content}
